...
- **`generic_listing` has no pagination.** Sopra Steria and Tieto both cap
  at exactly 10 postings (their page-1 size) — later pages are never fetched.
- **`generic_listing` doesn't escalate to Playwright when it should.**
...
- **Bouvet's ReachMee "job news feed" page** (`web106.reachmee.com/...`)
