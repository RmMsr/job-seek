...
**Tech Stack:** Python 3.12, uv (package manager), FastAPI 0.115, Jinja2, HTMX (CDN), httpx, BeautifulSoup4, Playwright 1.44, openai Python SDK 1.30, pytest 8, uvicorn, tomllib (stdlib)

## Global Constraints

- Python 3.12+ only (uses built-in `tomllib`)\n- `uv` for all package management — `uv sync` to install, `uv run` to invoke tools
...
```bash
uv run pytest tests/test_simplify.py -v
```
...
```bash
uv run pytest tests/test_classify.py tests/test_summarize.py tests/test_evaluate.py -v
```
...
```bash
uv run pytest tests/test_fetcher_http.py -v
```
...
```bash
uv run pytest tests/test_routes_jobs.py -v
```
...
```bash
uv run pytest tests/test_routes_fetch.py -v
```
...
```bash
uv run pytest tests/test_routes_profile.py tests/test_routes_scenarios.py -v
```
